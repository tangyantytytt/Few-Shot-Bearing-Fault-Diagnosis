...
# ------------------- 禁用 MetaAconC 以确保稳定性 -------------------
META_ACON_AVAILABLE = False
MetaAconC = None
CoordAtt = None
print("Using original implementations without MetaAconC for stability")
...
class MultiScaleExtractor(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.activation(x3 + x5 + x7)
        x = self.tail_pw(x) + x_copy

        x = self.pointwise(x)
        x = self.conv_cn(x)
        x = self.activation(self.norm_last(x))
        return x
...
class MahalanobisBlock(nn.Module):
    ...
    def mahalanobis_similarity(self, input, CovaMatrix_list, regularization=1e-6):
        ...
        for i in range(B):
            ...
            mea_sim = torch.zeros(1, len(CovaMatrix_list) * h * w, device=device)  # 使用相同设备
            
            for j in range(len(CovaMatrix_list)):
                # 确保协方差矩阵在正确的设备上
                covariance_matrix = CovaMatrix_list[j].float().to(device) + regularization * torch.eye(C, device=device)
...
class Ensemble_Net(nn.Module):
    ...
    def forward(self, input1, input2):
        ...
        # Lower branch - Mahalanobis距离分支
        m_l = self.covariance(q, S)
        
        # 确保分类器输入维度正确
        if m_l.dim() == 2:
            m_l = m_l.unsqueeze(1)  # 添加通道维度 (B, 1, L)
        
        m_l = self.classifier(m_l)
        m_l = m_l.squeeze(1)  # 移除通道维度 (B, L) -> (B, classes)

        # Upper branch - Transformer分支
    ...
    def get_features(self, input1):
        """提取输入图像的特征"""
        return self.features(input1)
    
    def get_global_features(self, input1):
        """提取全局平均池化后的特征向量（用于t-SNE可视化）"""
        features = self.features(input1)
        # 全局平均池化
        global_features = torch.nn.AdaptiveAvgPool2d((1, 1))(features)
        global_features = global_features.view(global_features.size(0), -1)
        return global_features
    
    def get_multilevel_features(self, input1):
        """提取多层级特征（用于更详细的分析）"""
        # 获取原始特征
        features = self.features(input1)
        ...
        # 不同层级的特征
        feature_dict = {
            'raw_features': features,  # 原始特征图
            'global_pooled': torch.nn.AdaptiveAvgPool2d((1, 1))(features).view(features.size(0), -1),  # 全局池化
            'spatial_mean': features.mean(dim=1).view(features.size(0), -1),  # 空间均值
            'channel_mean': features.mean(dim=[2, 3]).view(features.size(0), -1),  # 通道均值
        }
...
def test_model():
    ...
    # 创建测试输入（在CPU上测试）
    batch_size = 2
    query_img = torch.randn(batch_size, 1, 64, 64)
    support_imgs = [torch.randn(5, 1, 64, 64) for _ in range(10)]  # 10个类别，每个类别5个样本
    
    # 前向传播
    m_l, m_u, output = model(query_img, support_imgs)
    
    print(f"Query image shape: {query_img.shape}")
    print(f"Number of support classes: {len(support_imgs)}")
    print(f"Support images per class: {support_imgs[0].shape[0]}")
    print(f"Mahalanobis branch output shape: {m_l.shape}")
    print(f"Transformer branch output shape: {m_u.shape}")
    print(f"Final output shape: {output.shape}")
    
    # 测试特征提取功能
    print("\nTesting feature extraction...")
    features = model.get_features(query_img)
    global_features = model.get_global_features(query_img)
    multilevel_features = model.get_multilevel_features(query_img)
    
    print(f"Raw features shape: {features.shape}")
    print(f"Global features shape: {global_features.shape}")
    print("Multilevel features shapes:")
    for key, value in multilevel_features.items():
