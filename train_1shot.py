...
try:
    ...
    print("Warning: AdamP, LSR, or EarlyStopping modules not found. Using original implementations.")

parser = argparse.ArgumentParser(description='Bearing Faults Project Configuration')
parser.add_argument('--dataset', choices=['CWRU', 'PDB', 'PU'], help='Dataset (CWRU, PDB, or PU)')
...
parser.add_argument('--training_samples_PU', type=int, default=195, help='Number of training samples for PU')
...
parser.add_argument('--way_num_PU', type=int, default=13, help='Number of classes for PU')
...
parser.add_argument('--weight_decay', type=float, default=1e-4, help='Weight decay for AdamP')
parser.add_argument('--label_smoothing', type=float, default=0.1, help='Label smoothing factor')
parser.add_argument('--patience', type=int, default=10, help='Early stopping patience')
parser.add_argument('--use_optimizer', action='store_true', help='Use AdamP optimizer')
...
# 添加 loss1 的定义
parser.add_argument('--loss1', default=ContrastiveLoss(), help='First loss function')

args = parser.parse_args()
print(args)

# 优化：使用标签平滑
if OPTIMIZER_AVAILABLE and args.use_label_smoothing:
    args.loss2 = LSR(e=args.label_smoothing, reduction='mean')
else:
    args.loss2 = nn.CrossEntropyLoss()
...
if args.dataset == 'PU':
    window_size = 2048
    split = args.training_samples_PU // 13
    data = PU(split=split, length=window_size)
    
    data.X_train = data.X_train.astype(np.float32)
    data.X_test = data.X_test.astype(np.float32)
    
    # 限制训练样本数
    if len(data.X_train) > args.training_samples_PU:
        indices = np.random.RandomState(42).choice(len(data.X_train), 
                                                    args.training_samples_PU, 
                                                    replace=False)
        data.X_train = data.X_train[indices]
        data.y_train = data.y_train[indices]
    
    # 限制测试样本数
    if len(data.X_test) > 750:
        indices = np.random.RandomState(42).choice(len(data.X_test), 750, replace=False)
        data.X_test = data.X_test[indices]
        data.y_test = data.y_test[indices]
    
    train_data_PU = torch.from_numpy(data.X_train)
    train_label_PU = torch.from_numpy(data.y_train)
    test_data_PU = torch.from_numpy(data.X_test)
    test_label_PU = torch.from_numpy(data.y_test)
    
    train_data_PU = train_data_PU.reshape([train_data_PU.shape[0], 4096])
    test_data_PU = test_data_PU.reshape([test_data_PU.shape[0], 4096])
    
    train_data_PU = train_data_PU.reshape(train_data_PU.shape[0], 1, 64, 64)
    test_data_PU = test_data_PU.reshape(test_data_PU.shape[0], 1, 64, 64)
    
    print('Shape of PU train data:', train_data_PU.shape)
    print('Shape of PU test data:', test_data_PU.shape)
    print('End Loading PU')
    
    train_dataset_PU = FewshotDataset(train_data_PU, train_label_PU, 
                                       episode_num=args.episode_num_train, 
                                       way_num=args.way_num_PU, 
                                       shot_num=1, query_num=1)
    train_dataloader_PU = DataLoader(train_dataset_PU, batch_size=args.batch_size, shuffle=True)
    
    test_dataset_PU = FewshotDataset(test_data_PU, test_label_PU, 
                                      episode_num=args.episode_num_test, 
                                      way_num=args.way_num_PU, 
                                      shot_num=1, query_num=1)
    test_dataloader_PU = DataLoader(test_dataset_PU, batch_size=args.batch_size, shuffle=False)
...
def train_and_test_model_ensemble(net,
    ...
    if OPTIMIZER_AVAILABLE and args.use_optimizer:
        optimizer = AdamP(net.parameters(), lr=lr, weight_decay=args.weight_decay, 
                         nesterov=True, amsgrad=True)
        print("Using AdamP optimizer")
    else:
    ...
    if OPTIMIZER_AVAILABLE and args.use_early_stopping:
        early_stopping = EarlyStopping(patience=args.patience, verbose=True, 
                                     path=os.path.join(path_weight, 'best_model.pth'))
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        with torch.no_grad():
            ...
            if acc > pred_acc:
                ...
                torch.save(net, os.path.join(path_weight, model_name))
    ...
    history_path = os.path.join(path_weight, f'{args.model_name}_training_history.json')
...
if args.dataset == 'CWRU':
    full_loss, full_acc = train_and_test_model_ensemble(net,
    ...
    full_loss, full_acc = train_and_test_model_ensemble(net,
                        train_dataloader = train_dataloader_PU,
                        test_loader = test_dataloader_PU,
                        training_samples = args.training_samples_PU,
                        num_epochs = args.num_epochs,
                        lr = args.lr,
                        loss1 = args.loss1,
                        loss2 = args.loss2,
                        path_weight = args.path_weights,
                        num_samples = args.training_samples_PU)
    print('end training...................!!')